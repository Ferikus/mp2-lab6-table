...
int TAVLTree::LeftBalance(TTreeNode*& pNode)
{
	...
	case BalLeft:
	{
		...
		if (p1->bal == BalLeft) {
			...
			p2 = p1->pRight;
			p1->pRight = p2->pLeft; // p2 было nullptr
...
int TAVLTree::RightBalance(TTreeNode*& pNode)
{
	...
	case BalRight:
	{
		...
		if (p1->bal == BalRight) {
			...
			p2 = p1->pLeft;
			p1->pLeft = p2->pRight; // p2 было nullptr
...
{
	if (pNode == NULL) return NULL; // дерево пустое
	while (pNode->pLeft != NULL)
		pNode = pNode->pLeft; // идём влево, пока не дойдем до конца
	return pNode; // узел с минимальным ключом
}
